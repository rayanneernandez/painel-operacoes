...
export function normalizeFacialExpression(value) {
...
function increment(counts, expression) {
  if (!expression) return;
  counts[expression] += 1;
}

function applyStructuredEmotionValue(value, counts) {
...
function extractAdditionalAttributes(rawVisit) {
  const nested = rawVisit?.additional_atributes ?? rawVisit?.additional_attributes ?? [];
  return Array.isArray(nested) ? nested : [];
}

function extractCountsFromEntry(entry, counts) {
...
export function extractFacialExpressionCounts(rawVisit) {
  const counts = {
    neutral: 0,
    happiness: 0,
    surprise: 0,
...
export function getDominantFacialExpression(rawVisit) {
