    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-2 w-full min-w-0 min-h-0">
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-2">
          ...
          <span className="text-[11px] text-gray-400 mt-1 text-center leading-tight line-clamp-3 break-words" style={{ maxWidth: '62%' }}>{segments[0]?.label ?? ''}</span>
        </div>
...
export const WidgetCampaigns = ({
  ...
  // Mês selecionado — sincroniza com o filtro do dashboard
  const derivedMonth = React.useMemo(() => {
    const d = startDate ? new Date(startDate) : new Date();
    return new Date(d.getFullYear(), d.getMonth(), 1);
  }, [startDate]);
  const [selectedMonth, setSelectedMonth]   = React.useState<Date>(derivedMonth);
  const [rangeFromMonth, setRangeFromMonth] = React.useState<Date>(derivedMonth);
  const [showRangePicker, setShowRangePicker] = React.useState(false);
  // Picker temporário (só aplica ao fechar)
  const [pickerFrom, setPickerFrom] = React.useState<Date>(derivedMonth);
  const [pickerTo, setPickerTo]     = React.useState<Date>(derivedMonth);
  React.useEffect(() => {
    setSelectedMonth(derivedMonth);
    setRangeFromMonth(derivedMonth);
    setPickerFrom(derivedMonth);
    setPickerTo(derivedMonth);
  }, [derivedMonth]);
  ...
  // Compara apenas ano+mês, ignorando horário e fuso — evita falsos negativos
  const fromMonthNum = rangeFromMonth.getFullYear() * 12 + rangeFromMonth.getMonth();
  const toMonthNum   = selectedMonth.getFullYear()  * 12 + selectedMonth.getMonth();
  ...
  const filteredRows = React.useMemo(() => {
    return normalizedRows.filter((row) => {
      ...
      // Filtro de mês — mantém linha se start_date OU end_date estiver dentro do intervalo
      ...
      if (s || e) {
        const inRange = (d: Date | null) => {
          ...
          return m >= fromMonthNum && m <= toMonthNum;
        };
        if (!inRange(s) && !inRange(e)) return false;
      }
      return true;
    });
  ...
  const activeFilterCount = [campaignFilter, branchFilter].filter((value) => value !== 'all').length;
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 h-full min-h-0 flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Navegador de Mês com Range Picker */}
          <div className="relative inline-flex items-center">
            <div className="inline-flex items-center rounded-md border border-gray-800 bg-gray-950 overflow-hidden">
              {/* Seta anterior — move o mês de fim */}
              <button
                onClick={() => {
                  const prev = new Date(selectedMonth.getFullYear(), selectedMonth.getMonth() - 1, 1);
                  setSelectedMonth(prev);
                  // Se rangeFrom ficar à frente, ajusta também
                  if (rangeFromMonth > prev) setRangeFromMonth(prev);
                }}
                className="px-2 py-1 text-gray-500 hover:text-white hover:bg-gray-800 transition-colors text-[11px]"
                title="Mês anterior"
              >‹</button>

              {/* Rótulo clicável que abre o picker */}
              <button
                onClick={() => {
                  setPickerFrom(rangeFromMonth);
                  setPickerTo(selectedMonth);
                  setShowRangePicker(v => !v);
                }}
                className="px-2 py-1 text-[10px] text-gray-300 font-medium whitespace-nowrap hover:text-white transition-colors"
                title="Selecionar intervalo de meses"
              >
                {rangeFromMonth.getFullYear() === selectedMonth.getFullYear() && rangeFromMonth.getMonth() === selectedMonth.getMonth()
                  ? `${MESES_PT[selectedMonth.getMonth()]} ${selectedMonth.getFullYear()}`
                  : `${MESES_PT[rangeFromMonth.getMonth()]} ${rangeFromMonth.getFullYear()} – ${MESES_PT[selectedMonth.getMonth()]} ${selectedMonth.getFullYear()}`
                }
              </button>
            ...
            {/* Dropdown do range picker */}
            {showRangePicker && (() => {
              const years = [new Date().getFullYear() - 1, new Date().getFullYear()];
              const applyRange = () => {
                // Garante que from <= to
                const from = pickerFrom <= pickerTo ? pickerFrom : pickerTo;
                const to   = pickerFrom <= pickerTo ? pickerTo   : pickerFrom;
                setRangeFromMonth(from);
                setSelectedMonth(to);
                setShowRangePicker(false);
              };
              return (
                <div className="absolute top-full right-0 mt-1 z-50 rounded-lg border border-gray-700 bg-gray-900 shadow-xl p-3 min-w-[220px]">
                  <p className="text-[9px] text-gray-500 uppercase tracking-wide mb-2">Intervalo de meses</p>
                  {/* De */}
                  <div className="flex items-center gap-1 mb-2">
                    <span className="text-[10px] text-gray-400 w-6">De</span>
                    <select
                      value={pickerFrom.getMonth()}
                      onChange={e => setPickerFrom(new Date(pickerFrom.getFullYear(), Number(e.target.value), 1))}
                      className="flex-1 rounded border border-gray-700 bg-gray-800 text-[10px] text-gray-200 px-1 py-0.5"
                    >
                      {MESES_PT.map((m, i) => <option key={i} value={i}>{m}</option>)}
                    </select>
                    <select
                      value={pickerFrom.getFullYear()}
                      onChange={e => setPickerFrom(new Date(Number(e.target.value), pickerFrom.getMonth(), 1))}
                      className="rounded border border-gray-700 bg-gray-800 text-[10px] text-gray-200 px-1 py-0.5"
                    >
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    <span className="text-[10px] text-gray-400 w-6">Até</span>
                    <select
                      value={pickerTo.getMonth()}
                      onChange={e => setPickerTo(new Date(pickerTo.getFullYear(), Number(e.target.value), 1))}
                      className="flex-1 rounded border border-gray-700 bg-gray-800 text-[10px] text-gray-200 px-1 py-0.5"
                    >
                      {MESES_PT.map((m, i) => <option key={i} value={i}>{m}</option>)}
                    </select>
                    <select
                      value={pickerTo.getFullYear()}
                      onChange={e => setPickerTo(new Date(Number(e.target.value), pickerTo.getMonth(), 1))}
                      className="rounded border border-gray-700 bg-gray-800 text-[10px] text-gray-200 px-1 py-0.5"
                    >
                      {years.map(y => <option key={y} value={y}>{y}</option>)}
                    </select>
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={applyRange}
                      className="flex-1 rounded bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-medium py-1 transition-colors"
                    >Aplicar</button>
                    <button
                      onClick={() => setShowRangePicker(false)}
                      className="rounded border border-gray-700 bg-gray-800 hover:bg-gray-700 text-gray-400 text-[10px] px-2 py-1 transition-colors"
                    >✕</button>
                  </div>
                </div>
              );
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[10px] transition-colors ${
              showFilters || activeFilterCount > 0
                ...
                : 'border-gray-800 bg-gray-950 text-gray-300 hover:text-white'
            }`}
      ...
      <div className={`mb-3 grid grid-cols-1 gap-2 md:grid-cols-2 flex-shrink-0 ${showFilters ? '' : 'hidden'}`}>
        <select
