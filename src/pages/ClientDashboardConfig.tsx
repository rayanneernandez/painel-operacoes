...
export function ClientDashboardConfig() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        // 1. Tenta config do cliente
        const { data: clientConfig } = await supabase
          .from('dashboard_configs')
          .select('widgets_config')
          .eq('client_id', id)
        ...
        if (clientConfig?.widgets_config) {
          const wc = clientConfig.widgets_config;
          if (Array.isArray(wc)) ids = wc.filter((x: any) => typeof x === 'string');
          else if (Array.isArray(wc?.widget_ids)) ids = wc.widget_ids.filter((x: any) => typeof x === 'string');
        }
        ...
        if (!ids) {
          ...
          if (globalConfig?.widgets_config) {
            const wc = globalConfig.widgets_config;
            if (Array.isArray(wc)) ids = wc.filter((x: any) => typeof x === 'string');
          }
        }
        ...
        if (!ids || ids.length === 0) {
          ids = ['kpi_flow_stats', 'flow_trend', 'hourly_flow', 'age_pyramid', 'gender_dist', 'attributes', 'campaigns'];
        }

        if (cancelled) return;

        // Filtra apenas IDs que existem no AVAILABLE_WIDGETS
        const validIds = ids.filter((wid) => AVAILABLE_WIDGETS.some((w) => w.id === wid));
        const active    = validIds.map((wid) => AVAILABLE_WIDGETS.find((w) => w.id === wid)).filter(Boolean) as WidgetType[];
        const available = AVAILABLE_WIDGETS.filter((w) => !validIds.includes(w.id));
        ...
        const active    = defaultIds.map((wid) => AVAILABLE_WIDGETS.find((w) => w.id === wid)).filter(Boolean) as WidgetType[];
        const available = AVAILABLE_WIDGETS.filter((w) => !defaultIds.includes(w.id));
        if (!cancelled) { setActiveWidgets(active); setAvailableWidgets(available); }
      } finally {
  ...
  const addWidget    = (widget: WidgetType) => { setActiveWidgets([...activeWidgets, widget]); setAvailableWidgets(availableWidgets.filter((w) => w.id !== widget.id)); };
  const removeWidget = (widget: WidgetType) => { setAvailableWidgets([...availableWidgets, widget]); setActiveWidgets(activeWidgets.filter((w) => w.id !== widget.id)); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existing?.id) {
        await supabase
          .from('dashboard_configs')
          .update({ widgets_config: widgetIds, updated_at: new Date().toISOString() })
          .eq('id', existing.id);
      } else {
        await supabase
          .from('dashboard_configs')
          .insert({ client_id: id, layout_name: 'client', widgets_config: widgetIds, updated_at: new Date().toISOString() });
      }

      // Também salva no localStorage como cache local
      localStorage.setItem(`dashboard-config-${id}`, JSON.stringify(widgetIds));
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500 min-h-screen bg-gray-950 text-gray-100 font-sans">
      ...
      <div className="flex items-center justify-between border-b border-gray-800 pb-6">
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white px-6 py-2.5 rounded-lg flex items-center gap-2 font-medium transition-all shadow-lg shadow-emerald-900/20"
        >
          <Save size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider">Layout Ativo (Rede Selecionada)</h2>
              ...
              <div key={widget.id} className="relative bg-gray-900 border border-gray-800 p-4 rounded-xl flex items-center gap-4 group hover:border-emerald-500/30 transition-all shadow-sm z-10">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-white text-sm">{widget.title}</h3>
                    {widget.type === 'chart' && <span className="w-2 h-2 rounded-full bg-blue-500" />}
                    {widget.type === 'table' && <span className="w-2 h-2 rounded-full bg-purple-500" />}
                    {widget.type === 'kpi'   && <span className="w-2 h-2 rounded-full bg-orange-500" />}
                  </div>
                  <p className="text-xs text-gray-500 mt-1">{widget.description}</p>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex flex-col gap-1 mr-2">
                    <button onClick={() => moveWidget(index, 'up')} disabled={index === 0} className="p-1 rounded bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700 disabled:opacity-30 transition-colors"><ArrowUp size={14} /></button>
                    <button onClick={() => moveWidget(index, 'down')} disabled={index === activeWidgets.length - 1} className="p-1 rounded bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700 disabled:opacity-30 transition-colors"><ArrowDown size={14} /></button>
                  </div>
