-- ============================================================================
-- content_rollup: EXPRESSÕES FACIAIS por conteúdo (neutro/felicidade por hora)
-- ----------------------------------------------------------------------------
-- Adiciona expr_hour_counts (jsonb) e atualiza a função de upsert.
-- Formato de expr_hour_counts: { "14:happiness": 3, "14:neutral": 5, ... }
--   chave = "hora:expressao"  (hora 0-23, expressao = neutral|happiness)
-- Alimentado pelo import de "Views of visitors" (coluna History face attributes).
-- Seguro rodar mesmo com a tabela já existindo. Supabase → SQL Editor → Run.
-- ============================================================================

ALTER TABLE content_rollup ADD COLUMN IF NOT EXISTS expr_hour_counts jsonb NOT NULL DEFAULT '{}'::jsonb;
...
BEGIN
  INSERT INTO content_rollup (
    ...
    glasses_counts, facial_counts, haircolor_counts, hairtype_counts, expr_hour_counts
  )
  SELECT
    (r->>'client_id')::uuid,
    r->>'content_name',
    ...
    COALESCE(r->'expr_hour_counts', '{}'::jsonb)
  ...
  ON CONFLICT (client_id, content_name, day) DO UPDATE SET
    ...
    expr_hour_counts = EXCLUDED.expr_hour_counts,
    updated_at = now();
